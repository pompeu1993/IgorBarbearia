...
function DateTimeSelection() {
    ...
    const handleConfirmReschedule = async () => {
        if (!finalDateTimeStr || !appointmentId) return;

        const newApptDate = new Date(finalDateTimeStr).getTime();
        const now = new Date().getTime();
        
        if (newApptDate <= now) {
            alert("Não é possível agendar para um horário no passado.");
            return;
        }

        const { error } = await supabase
            .from("appointments")
            .update({ date: finalDateTimeStr })
            .eq("id", appointmentId);
        ...
        <>
            <div className="sticky top-0 z-30 bg-black/90 backdrop-blur-xl border-b border-white/5 p-4">
                <div className="flex items-center justify-between mx-auto">
                    ...
                    <h2 className="text-white text-lg font-bold tracking-tight">Reagendar Horário</h2>
            ...
            <div className={`fixed bottom-[72px] left-0 right-0 bg-black/95 backdrop-blur-2xl border-t border-white/5 p-5 z-40 max-w-md mx-auto pb-6 transition-transform duration-500 ${selectedDate && selectedTime ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="space-y-4">
                    ...
                    <button onClick={handleConfirmReschedule} className="w-full h-16 bg-gradient-to-r from-[#dca715] via-primary to-[#dca715] text-black rounded-2xl font-black text-lg shadow-[0_10px_30px_-10px_rgba(212,175,55,0.5)] flex items-center justify-center gap-3 active:scale-[0.98] transition-transform">
